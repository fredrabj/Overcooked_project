...
def update_policy_clipped(policy, optimizer, observations, actions, old_log_probs, advantages, clip_eps=0.2):
    with tf.GradientTape() as tape:
        ...
        clipped_ratio = tf.clip_by_value(ratio, 1.0 - clip_eps, 1.0 + clip_eps)
...
def compute_gae_tf(rewards, values, next_values, gamma=0.99, lam=0.95):
    deltas = rewards + gamma * next_values - values
    gae = tf.zeros_like(rewards)
    advs = []
...
def main():
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            action1_tf, logprob_1 = compute_action_tf(obs1_tensor, agent_1_policy)
            action2_tf, logprob_2 = compute_action_tf(obs2_tensor, agent_2_policy)
            ...
            logp1_list.append(logprob_1.numpy())

            obs2_list.append(obs2)
            act2_list.append(action2)
            rew2_list.append(rew2)
            logp2_list.append(logprob_2.numpy())
        ...
        next_values = tf.concat([values[1:], tf.expand_dims(last_value, 0)], axis=0)

        advantages = compute_gae_tf(rewards_tensor, values, next_values, gamma=gamma, lam=0.95)
        ...
        logp1_tensor = tf.convert_to_tensor(logp1_list, dtype=tf.float32)
        logp2_tensor = tf.convert_to_tensor(logp2_list, dtype=tf.float32)

        loss1 = update_policy_clipped(agent_1_policy, actor_optimizer_1, obs1_tensor, act1_tensor, logp1_tensor, advantages)
        loss2 = update_policy_clipped(agent_2_policy, actor_optimizer_2, obs2_tensor, act2_tensor, logp2_tensor, advantages)
